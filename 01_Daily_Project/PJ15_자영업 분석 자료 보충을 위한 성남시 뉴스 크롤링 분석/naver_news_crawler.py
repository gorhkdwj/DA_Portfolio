...
def _next_date(cur: datetime, interval_size: int, interval_unit: str) -> datetime:
    ...
    raise ValueError(
        f"지원하지 않는 interval_unit: '{interval_unit}' "
...
# ---------------------------------------------------------------------------
def append_to_csv(
    items: list[dict], filepath: Path, write_header: bool = False
) -> None:
...
def crawl_segment(
    ...
    while True:
        ...
        if not page_items:
            logger.info(
                f"    페이지 {page}: 네이버뉴스 기사 없음 (외부 언론사 페이지). 건너뜀."
            )
            page += 1
            continue
...
def run_full_crawl(
    ...
    # ── 체크포인트 경로 (키워드 기준 고정) ──────────────────────────────────
    checkpoint_path = output_dir / f"{keyword}_checkpoint.json"

    # ── 체크포인트 복원 또는 신규 시작 ──────────────────────────────────────
    if checkpoint_path.exists():
        ckpt = json.loads(checkpoint_path.read_text(encoding="utf-8"))
        filepath     = Path(ckpt["filepath"])
        done_indices = set(ckpt["done_indices"])   # 완료된 구간 인덱스 집합
        total_count  = ckpt["total_count"]

        # 기존 CSV에서 seen_links 복원
        ...
        logger.info(
            f"[{keyword}] 체크포인트 복원 — "
            f"완료 구간 {len(done_indices)}/{len(segments)}개, "
            f"기수집 {total_count}건, 저장 경로: {filepath}"
        )
    else:
        ...
        logger.info(
            ...
            f"구간 {interval_size}{interval_unit} 단위, 총 {len(segments)}개 구간 | "
            f"저장 경로: {filepath}"
        )
    ...
    try:
        ...
        for idx, (seg_start, seg_end) in enumerate(segments, start=1):
            if idx in done_indices:
                logger.info(f"[구간 {idx}/{len(segments)}] 이미 완료 — 건너뜀.")
                continue
    ...
    # 전체 완료 시 체크포인트 삭제
    if len(done_indices) == len(segments):
        checkpoint_path.unlink(missing_ok=True)
        logger.info("체크포인트 삭제 완료 (전체 수집 완료).")
